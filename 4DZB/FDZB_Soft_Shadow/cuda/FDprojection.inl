...
namespace FD{
	...
	__host__ __device__ __inline__ void makeRectangle(float3 & rect, const float * const upRightMat, const float3 &tVector)
	{
	...
	__host__ __device__ __inline__ void getZ(float &z, const float * const upRightMat, const float3 &tVector)
	{
		z = upRightMat[3] * tVector.x + upRightMat[7] * tVector.y + upRightMat[11] * tVector.z + upRightMat[15] * 1.0f;
	}
